...
_PNG = (
    Opt("compression", "Compression", "int", 6, vips="compression", lo=0, hi=9,
        hint="zlib level. 6 is a good balance, 9 is slow for ~1% smaller files."),
    Opt("effort", "Palette effort", "int", 7, vips="effort", lo=1, hi=10, advanced=True,
        only_if=("palette", True), hint="Quantisation effort when writing a palette PNG."),
    Opt("interlace", "Interlaced (Adam7)", "bool", False, vips="interlace", advanced=True,
        hint="Progressive display, slightly larger files."),
    Opt("palette", "Indexed palette", "bool", False, vips="palette",
    ...
    Opt("bitdepth", "Palette bit depth", "choice", 8, vips="bitdepth", advanced=True,
        only_if=("palette", True),
        choices=(("8 bit (256)", 8), ("4 bit (16)", 4), ("2 bit (4)", 2), ("1 bit (2)", 1))),
    Opt("dither", "Dither", "float", 1.0, vips="dither", lo=0, hi=1, step=0.1, advanced=True,
        only_if=("palette", True)),
)

_JPEG = (
    Opt("Q", "Quality", "int", 92, vips="Q", lo=1, hi=100),
    Opt("subsample_mode", "Chroma subsampling", "choice", "off", vips="subsample_mode",
        choices=(("4:4:4 (no subsampling)", "off"), ("Auto (4:2:0 below Q90)", "auto"),
                 ("4:2:0 (always)", "on")),
        hint="4:4:4 keeps coloured text and screentones clean."),
    Opt("optimize_coding", "Optimize Huffman tables", "bool", True, vips="optimize_coding",
        hint="Smaller files, no quality change."),
    Opt("interlace", "Progressive", "bool", True, vips="interlace"),
    Opt("trellis_quant", "Trellis quantisation", "bool", False, vips="trellis_quant",
        advanced=True, hint="mozjpeg feature: smaller files, slower encode."),
    Opt("overshoot_deringing", "Overshoot deringing", "bool", False, vips="overshoot_deringing",
        advanced=True, hint="Reduces ringing around hard black edges."),
    Opt("optimize_scans", "Optimize progressive scans", "bool", False, vips="optimize_scans",
        advanced=True, only_if=("interlace", True)),
    Opt("quant_table", "Quant table", "choice", 3, vips="quant_table", advanced=True,
        choices=(("0 - JPEG Annex K", 0), ("1 - Flat", 1), ("2 - Custom MSSIM", 2),
                 ("3 - ImageMagick", 3), ("4 - Custom PSNR-HVS", 4), ("5 - Klein", 5),
                 ("6 - Watson", 6), ("7 - Ahumada", 7), ("8 - Peterson", 8))),
)

_WEBP = (
    Opt("Q", "Quality", "int", 90, vips="Q", lo=1, hi=100, not_if=("lossless", True)),
    Opt("lossless", "Lossless", "bool", False, vips="lossless"),
    Opt("effort", "Effort", "int", 4, vips="effort", lo=0, hi=6,
        hint="0 = fastest, 6 = smallest. Encode time grows quickly."),
    Opt("preset", "Preset", "choice", "drawing", vips="preset",
        choices=(("Default", "default"), ("Picture", "picture"), ("Photo", "photo"),
                 ("Drawing", "drawing"), ("Icon", "icon"), ("Text", "text")),
        hint="Drawing/Text suit manga and line art."),
    Opt("smart_subsample", "Smart subsampling", "bool", True, vips="smart_subsample",
        not_if=("lossless", True)),
    Opt("near_lossless", "Near-lossless", "bool", False, vips="near_lossless", advanced=True,
        only_if=("lossless", True), hint="Lossless container, Q used as pre-processing strength."),
    Opt("alpha_q", "Alpha quality", "int", 100, vips="alpha_q", lo=0, hi=100, advanced=True),
    Opt("min_size", "Minimise size", "bool", False, vips="min_size", advanced=True,
        hint="Slow exhaustive search for the smallest file."),
)

_AVIF = (
    Opt("Q", "Quality", "int", 63, vips="Q", lo=1, hi=100, not_if=("lossless", True),
        hint="AVIF quality is not comparable to JPEG: 55-70 is usually visually clean."),
    Opt("lossless", "Lossless", "bool", False, vips="lossless"),
    Opt("effort", "Effort", "int", 4, vips="effort", lo=0, hi=9,
        hint="0 = fastest, 9 = smallest. AVIF encoding is CPU heavy."),
    Opt("bitdepth", "Bit depth", "choice", 8, vips="bitdepth",
        choices=(("8 bit", 8), ("10 bit", 10), ("12 bit", 12)), advanced=True),
    Opt("subsample_mode", "Chroma subsampling", "choice", "off", vips="subsample_mode",
        choices=(("4:4:4 (no subsampling)", "off"), ("Auto", "auto"), ("4:2:0", "on")),
        advanced=True),
    Opt("encoder", "Encoder", "choice", "auto", vips="encoder", advanced=True,
        choices=(("Auto", "auto"), ("aom", "aom"), ("rav1e", "rav1e"), ("svt", "svt"))),
)

_JXL = (
    Opt("rate_mode", "Rate control", "choice", "quality", vips=None,
        choices=(("Quality (0-100)", "quality"), ("Butteraugli distance", "distance")),
        not_if=("lossless", True)),
    Opt("Q", "Quality", "int", 90, vips="Q", lo=1, hi=100,
        only_if=("rate_mode", "quality"), not_if=("lossless", True)),
    Opt("distance", "Distance", "float", 1.0, vips="distance", lo=0.1, hi=15.0, step=0.1,
        only_if=("rate_mode", "distance"), not_if=("lossless", True),
    ...
    Opt("effort", "Effort", "int", 7, vips="effort", lo=3, hi=9,
        hint="3 = fast, 7 = default, 9 = slowest and smallest."),
    Opt("tier", "Decode speed tier", "int", 0, vips="tier", lo=0, hi=4, advanced=True,
        hint="Higher decodes faster at the cost of density."),
)

FORMATS: dict[str, Fmt] = {
    "png": Fmt("png", "PNG", ".png", ".png", _PNG, probe="pngsave",
               hint="Lossless, universally supported, biggest files."),
    "jpeg": Fmt("jpeg", "JPEG", ".jpg", ".jpg", _JPEG, probe="jpegsave",
                hint="Lossy 8-bit, fastest to encode and read anywhere."),
    "webp": Fmt("webp", "WEBP", ".webp", ".webp", _WEBP, probe="webpsave",
                hint="Good size/quality, wide reader support."),
    "avif": Fmt("avif", "AVIF", ".avif", ".avif", _AVIF, probe="heifsave",
                hint="Smallest lossy files, slow encode, newer readers only."),
    "jxl": Fmt("jxl", "JPEG XL", ".jxl", ".jxl", _JXL, probe="jxlsave",
...
def is_active(opt: Opt, values: dict[str, Any]) -> bool:
    ...
    if opt.only_if is not None and not _matches(opt.only_if, values):
        return False
    if opt.not_if is not None and _matches(opt.not_if, values):
        return False
    return True
