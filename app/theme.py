...
class Theme:
    ...
    def apply(self, mode: str) -> None:
        ...
        s.configure(".", background=p.bg, foreground=p.text, font=f["body"],
        ...
        s.configure("Chip.TLabel", background=p.surface2, foreground=p.muted,
                    font=f["tiny"], padding=(6, 2))
        ...
        s.map("Ghost.TButton",
              background=[("active", p.surface2)], foreground=[("active", p.text)])
        s.configure("Link.TButton", background=p.bg, foreground=p.accent, padding=(4, 2),
                    font=f["small"])
        ...
        s.configure("Seg.Toolbutton", background=p.surface2, foreground=p.muted,
                    padding=(12, 5), anchor="center", font=f["small"], relief="flat",
                    borderwidth=0)
        s.map("Seg.Toolbutton",
              background=[("selected", p.accent), ("active", p.border)],
              foreground=[("selected", p.accent_text), ("active", p.text)])

        s.configure("TCheckbutton", background=p.surface, foreground=p.text,
                    indicatorcolor=p.surface2, padding=(0, 3))
        s.map("TCheckbutton",
              background=[("active", p.surface)],
              indicatorcolor=[("selected", p.accent), ("pressed", p.accent_hi)],
              foreground=[("disabled", p.muted)])
        s.configure("Inset.TCheckbutton", background=p.surface2, foreground=p.text,
                    indicatorcolor=p.surface)
        s.map("Inset.TCheckbutton", background=[("active", p.surface2)],
              indicatorcolor=[("selected", p.accent)])

        s.configure("TEntry", fieldbackground=p.surface2, foreground=p.text,
                    bordercolor=p.border, lightcolor=p.surface2, darkcolor=p.surface2,
                    insertcolor=p.text, padding=(6, 5))
        s.map("TEntry", bordercolor=[("focus", p.accent)],
              fieldbackground=[("disabled", p.surface)], foreground=[("disabled", p.muted)])

        s.configure("TSpinbox", fieldbackground=p.surface2, foreground=p.text,
                    bordercolor=p.border, arrowcolor=p.muted, insertcolor=p.text,
                    lightcolor=p.surface2, darkcolor=p.surface2, padding=(6, 4))
        s.map("TSpinbox", bordercolor=[("focus", p.accent)],
              arrowcolor=[("active", p.text), ("disabled", p.border)],
              foreground=[("disabled", p.muted)])

        s.configure("TCombobox", fieldbackground=p.surface2, background=p.surface2,
                    foreground=p.text, bordercolor=p.border, arrowcolor=p.muted,
                    lightcolor=p.surface2, darkcolor=p.surface2, padding=(6, 4),
                    selectbackground=p.surface2, selectforeground=p.text)
        s.map("TCombobox", bordercolor=[("focus", p.accent)],
              arrowcolor=[("active", p.text), ("disabled", p.border)],
              fieldbackground=[("readonly", p.surface2), ("disabled", p.surface)],
              foreground=[("disabled", p.muted)])

        s.configure("TProgressbar", background=p.accent, troughcolor=p.surface2,
                    bordercolor=p.surface2, lightcolor=p.accent, darkcolor=p.accent,
                    thickness=6)
        s.configure("TSeparator", background=p.border)
        s.configure("TScale", background=p.surface, troughcolor=p.surface2)
        s.configure("Vertical.TScrollbar", background=p.surface2, troughcolor=p.bg,
                    bordercolor=p.bg, arrowcolor=p.muted, darkcolor=p.surface2,
                    lightcolor=p.surface2)
        s.map("Vertical.TScrollbar", background=[("active", p.border)])
        s.configure("Horizontal.TScrollbar", background=p.surface2, troughcolor=p.bg,
                    bordercolor=p.bg, arrowcolor=p.muted, darkcolor=p.surface2,
                    lightcolor=p.surface2)
